  ...
  const nextStep = () => {
    ...
    if (step === 2) {
      ...
      if (selectedDateTime < now) {
        setError(
          "The selected date and time have already passed. Please choose a future time.",
        );
        return;
      }
      ...
      if (selectedDateTime < sixHoursLater) {
        setError(
          "For operational reasons, bookings must be made at least 6 hours in advance. Please choose a later time.",
        );
        return;
      }
    }
      ...
      if (result !== null) {
        if (status === "OK") {
          ...
          if (status === "ZERO_RESULTS") {
            ...
            setError(
              "One of the addresses could not be found. Please check your entry.",
            );
          } else {
            setError(
              "Could not calculate directions. Please try a different address.",
            );
          }
          setDirections(null);
          setDistance("");
  ...
  const handleVerifyFlight = async () => {
    ...
    try {
      ...
      if (info) {
        ...
        setError(
          "Could not find flight information. Please check the number and date.",
        );
        setFlightInfo(null);
      }
    } catch (err) {
        ...
        <div className="space-y-6">
          {error && (
            <motion.div
              ...
              className="p-4 bg-red-500/10 border border-red-500/20 text-red-500 text-sm flex items-center gap-3 rounded-lg"
            >
              <AlertCircle size={18} />
              {error}
            </motion.div>
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 2 && (
              <motion.div
                ...
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: -20 }}
                className="glass p-4 rounded-lg md:p-12 grid grid-cols-1 lg:grid-cols-2 gap-8 items-start"
              >
                <div className="space-y-6">
                        ...
                        <div className="relative flex-1">
                          ...
                          >
                            <input
                              ...
                              className="w-full bg-white/5 rounded-lg border border-white/10 py-3 pl-12 pr-4 focus:border-gold outline-none transition-all text-sm"
                              value={wp}
                              onChange={(e) => {
                                const newWps = [...formData.waypoints];
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            updateForm("hours", parseInt(e.target.value))
                          }
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative h-[300px] md:h-[400px] lg:h-[500px] rounded-xl overflow-hidden border border-white/10 shadow-2xl">
                    <GoogleMap
                    ...
                    {filteredFleet.map((v) => {
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            "flex flex-col p-4 border rounded-lg transition-all text-left overflow-hidden group",
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex gap-4">
                              <div className="flex items-center gap-2 text-white/60 text-[10px] uppercase tracking-widest font-bold">
                                <User size={12} className="text-gold" /> {v.pax}{" "}
                                Pax
                              </div>
                              <div className="flex items-center gap-2 text-white/60 text-[10px] uppercase tracking-widest font-bold">
