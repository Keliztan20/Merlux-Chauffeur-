...
import { MapPin, Clock, Users, ArrowRight, Star, Calendar, User, Mail, Phone, CreditCard, Minus, Plus, ChevronRight, CheckCircle, Info, LayoutGrid, DollarSign, Tag, X, Image as ImageIcon, Banknote as BanknoteIcon, UserCheck, FileText, Luggage } from 'lucide-react';
...
export default function Tours() {
  ...
  const handleStartBooking = () => {
    ...
    if (!selectedFleet && selectedTour?.fleets && selectedTour.fleets.length > 0) {
      setSelectedFleet(selectedTour.fleets[0]);
    }
  };
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                ...
                <div className="lg:col-span-8 space-y-12">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {(selectedTour?.fleets && Array.isArray(selectedTour.fleets) ? selectedTour.fleets : []).map((f: any, idx: number) => {
                      ...
                      <div className="space-y-4 mb-6 glass p-5 rounded-2xl border border-white/5">
                        ...
                        <div className="space-y-3">
                          {Object.entries(selectedExtras).map(([idOrName, count], idx) => {
                            const extra = (selectedTour?.extras || []).find((e: any) => (e.id || e.name) === idOrName);
                            return (
                              <div key={`extra-summary-${idx}`} className="flex justify-between items-center text-[11px] border-b border-white/5 pb-2 last:border-0 last:pb-0">
                    ...
                    <div className="space-y-4 mb-10 glass p-6 rounded-3xl border border-white/5">
                      ...
                      {Object.keys(selectedExtras).length > 0 && (
                        <div className="space-y-2 border-b border-white/5 pb-3">
                          ...
                          {Object.entries(selectedExtras).map(([idOrName, count], idx) => {
                            const extra = (selectedTour?.extras || []).find((e: any) => (e.id || e.name) === idOrName);
                            return <p key={idx} className="text-[10px] text-white/60">• {extra?.name} (x{count})</p>
                          })}
                        </div>
                      )}
