...
function AppLayout() {
  ...
  if (isMobile)
    toastWidth = toastSettings?.width?.mobile || "calc(100% - 32px)";
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans selection:bg-gold selection:text-black flex flex-col">
      <Toaster
        ...
        toastOptions={{
          className:
            "glass !text-white !rounded-2xl !font-bold !tracking-[0.1em] !uppercase !backdrop-blur-xl",
          duration: toastSettings?.duration || 5000,
          style: {
            background:
              toastSettings?.bgColors?.default || "rgba(0, 0, 0, 0.8)",
            color: "#fff",
            border: `1px solid ${toastSettings?.colors?.default || "rgba(212, 175, 55, 0.3)"}`,
            backdropFilter: "blur(12px)",
            width: toastWidth,
            padding: toastSettings?.padding
              ? `${toastSettings.padding}px`
              : "16px",
            maxWidth: "100%",
          },
          success: {
            ...
            style: {
              background:
                toastSettings?.bgColors?.success || "rgba(0, 0, 0, 0.8)",
          ...
          error: {
            ...
            style: {
              background:
                toastSettings?.bgColors?.error || "rgba(0, 0, 0, 0.8)",
